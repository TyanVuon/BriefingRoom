...
class ImapConnector(
    ...
    def _get_mail_client(self) -> imaplib.IMAP4_SSL:
        ...
        host = (self._host or "").strip() or "outlook.office365.com"
        try:
            port = int(self._port)
        except (TypeError, ValueError):
            port = _DEFAULT_IMAP_PORT_NUMBER

        mail_client = imaplib.IMAP4_SSL(host=host, port=port)
        self._authenticate_mail_client(mail_client, username=username, password=password)
        return mail_client

    @staticmethod
    def _authenticate_mail_client(
        mail_client: imaplib.IMAP4_SSL,
        *,
        username: str,
        password: str,
    ) -> None:
        """Authenticate using the mechanism the server supports (PLAIN before LOGIN)."""
        _, cap_data = mail_client.capability()
        capabilities = b" ".join(cap_data or []).upper()

        if b"AUTH=PLAIN" in capabilities or b"AUTH PLAIN" in capabilities:
            def _plain_response(_challenge: bytes) -> bytes:
                return f"\0{username}\0{password}".encode()

            try:
                status, _data = mail_client.authenticate("PLAIN", _plain_response)
