...
class Graph:
    ...
    def __init__(self, dsl: str, tenant_id=None, task_id=None, custom_header=None):
        ...
        parsed = normalize_chunker_dsl(json.loads(dsl))
        try:
            from common.vault_runtime import maybe_inject_dsl_from_vault

            maybe_inject_dsl_from_vault(parsed, tenant_id)
        except Exception as exc:
            logging.getLogger(__name__).debug("Vault DSL injection skipped: %s", exc)
        self.dsl = parsed
