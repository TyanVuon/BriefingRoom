...
async def update_connector(connector_id):
    ...
    if req:
        ...
        if "config" in update_fields and isinstance(update_fields["config"], dict):
            incoming = update_fields["config"]
            existing = conn.config or {}
            merged = {**existing, **incoming}
            incoming_creds = incoming.get("credentials")
            existing_creds = existing.get("credentials") or {}
            if isinstance(incoming_creds, dict):
                merged_creds = {**existing_creds, **incoming_creds}
                for key, value in incoming_creds.items():
                    if isinstance(value, str) and not value.strip() and key in existing_creds:
                        merged_creds[key] = existing_creds[key]
                merged["credentials"] = merged_creds
...
async def test_connector(connector_id):
    ...
    if conn.source == DocumentSource.IMAP:
        from common.data_source.imap_connector import ImapConnector

        host = (config.get("imap_host") or "").strip() or "outlook.office365.com"
        try:
            port = int(config.get("imap_port") or 993)
        except (TypeError, ValueError):
            port = 993
        mailboxes = config.get("imap_mailbox") or None

        connector = ImapConnector(host=host, port=port, mailboxes=mailboxes)
        ...
        try:
            ...
            if "AUTHENTICATE failed" in msg or "authentication failed" in msg.lower():
                detail = (
                    f"TLS connected to {host}:{port} but IMAP login was rejected by the server. "
                    "Port and host look fine — check Outlook Settings → Mail → Sync email → "
            ...
            return get_json_result(
                code=RetCode.DATA_ERROR,
                message=detail,
                data={"host": host, "port": port, "tls_ok": True, "auth_ok": False},
            )
        return get_json_result(
            data={"host": host, "port": port, "tls_ok": True, "auth_ok": True},
        )

    return get_json_result(
        code=RetCode.ARGUMENT_ERROR,
        message="Test endpoint currently supports REST API and IMAP connectors.",
        data=False,
    )
