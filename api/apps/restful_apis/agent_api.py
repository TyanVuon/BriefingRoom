...
from common.agent_privacy import ephemeral_sessions_enabled
...
async def _run_workflow_session(
    ...
    dsl_for_privacy = workflow_conv.get("dsl") or str(canvas)
    ephemeral = ephemeral_sessions_enabled(dsl_for_privacy)
    ephemeral_purged = False

    async def purge_ephemeral_session():
        nonlocal ephemeral_purged
        if ephemeral and not ephemeral_purged:
            await thread_pool_exec(API4ConversationService.delete_by_id, session_id)
            ephemeral_purged = True

    async def persist_workflow_session():
        if not final_ans:
            await purge_ephemeral_session()
            return
...
async def create_agent_session(agent_id, tenant_id):
    ...
    session_name = req.get("name", "")
    if ephemeral_sessions_enabled(dsl):
...
async def agent_chat_completion(tenant_id, agent_id=None):
    ...
    dsl_for_privacy = None
    if session_id and exists:
        dsl_for_privacy = conv.dsl
    if dsl_for_privacy is None and agent_id:
        try:
            _, dsl_for_privacy = await thread_pool_exec(
                UserCanvasService.get_agent_dsl_with_release,
                agent_id,
                bool(req.get("release", False)),
                tenant_id,
            )
        except LookupError:
            dsl_for_privacy = None
    if dsl_for_privacy is not None:
        from api.apps.restful_apis.privacy_api import (
            require_hardware_auth_for_dsl,
            require_local_vault_for_dsl,
        )

        ok, hw_msg = require_hardware_auth_for_dsl(dsl_for_privacy)
        if not ok:
            return get_json_result(data=False, message=hw_msg, code=RetCode.OPERATING_ERROR)
        ok, vault_msg = require_local_vault_for_dsl(dsl_for_privacy)
        if not ok:
            return get_json_result(data=False, message=vault_msg, code=RetCode.OPERATING_ERROR)
