...
import { ephemeralSessionsEnabled } from '@/pages/agent/utils/ephemeral-session';
...
export const useSendSessionMessage = () => {
  ...
  const handlePressEnter = useCallback(async () => {
    ...
    if (isEmpty(sessionId) && canvasId) {
      ...
      try {
        const ephemeral = ephemeralSessionsEnabled(canvasInfo?.dsl);
        const sessionName = ephemeral
          ? 'New Session'
          : value?.trim() || 'New Session';
